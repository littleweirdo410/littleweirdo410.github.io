...
Десктопное приложение для ОС Windows, предназначенное для визуализации данных о музыкальных фольклорных мероприятиях в рамках городского фестиваля "Московская Масленица" с 2018 по 2020 гг.
...
Если начинающего Python-разработчика вдруг посещает смелая мысль о создании своего собственного десктопного приложения, как правило, он выбирает из двух доступных опций: **TKinter** и **PyQT** (вообще-то существует ещё и **Customtkinter**, нечто среднее между этими двумя парнями в плане красоты получающегося интерфейса, но в годы создания "Фольклора 2.0" я о нём не слышала). Эти две библиотеки для создания графических пользовательских интерфейсов (GUI) имеют несколько ключевых сходств и отличий, позволяющих сделать взвешенный и рациональный выбор в каждом отдельном случае. Рассмотрим оба фреймворка подробнее:
...
Скомпилировано под ОС Windows 10 (64 bit) с помошью pyinstaller

Готовый к использованию дистрибутив приложения с кратким руководством пользователя и небольшой видеоинструкцией по применению прикреплён к репозиторию на Github в разделе Releases https://github.com/littleweirdo410/folklore_2.0_desktop/releases/tag/latest
