...
>   * <a style="color:#ffffff">В основании функции лежит обычный цикл ```for```, проходящийся по новостным фрагментам из датасета. После пословной разбивки отрывка для каждого слова определялись нормальная (начальная) форма и часть речи. Для лемматизации (приведения к нормальной форме) и автоматического определения части речи (part of speech, сокращённо POS) были использованы библиотека Pymorphy2 и токенизатор на регулярных выражениях из библиотеки NLTK. Далее в зависимости от полученной части речи выбирался соответствующий словарь, где ключами были сами слова в нормальной форме, а значениями -- количества их вхождений во все обработанные на данный момент фрагменты. Если что-то ранее не попадалось в тексте, то оно заносилось в словарь как новый ключ со значением 1, если же уже встречалось, то к значению соответствующего ключа прибавлялась единица. Словари были заведены только для необходимых нам для исследования частей речи (имён существительных, имён прилагательных и глаголов) -- слова, получившие иные метки POS, после обработки Pymorphy2 просто игнорировались, поэтому алгоритм получился довольно громоздким и долго выполняющимся, особенно на таком непомерно объёмном корпусе. Кроме механизма автосохранения прогресса и самого цикла для отслеживания изменений, мной был также реализован формат выходных данных.</a>
