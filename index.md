...
>   * <a style="color:#ffffff">Кроме того, я предусмотрела несколько ячеек и кое-что внутри самой функции обработки новостного корпуса для того, чтобы иметь возможность на время приостанавливать процесс подсчёта частотности с сохранением прогресса и записью номера отрывка, на котором процесс был приостановлен, а также осуществлять резервное копирование. Впоследствии в ходе тестирования полученного Франкенштейна перед началом подсчёта эти же ячейки пару раз использовались с целью обнуления сохранённой статистики и текущей позиции в датасете, если что-то шло не так. Система была устроена довольно просто и основывалась на бинарных файлах и встроенной библиотеке pickle;</a>
...
>   * <a style="color:#ffffff">В основании функции лежит обычный цикл ```for```, проходящийся по новостным фрагментам из датасета. Отрывок разбивался на отдельные слова, после чего для каждого слова определялись нормальная (начальная) форма и часть речи. Для лемматизации (приведения слов к нормальной форме) и автоматического определения части речи (part of speech, POS) были использованы библиотека Pymorphy2 и токенизатор на регулярных выражениях из библиотеки NLTK. Далее в зависимости от полученной части речи выбирался соответствующий словарь, где ключами были слова в нормальной форме, а значениями -- количества их вхождений во все обработанные на данный момент фрагменты. Если слово ранее не попадалось в тексте, то оно заносилось в словарь как новый ключ со значением 1, если же слово уже встречалось, то к значению соответствующего ключа прибавлялась единица. Словари были заведены только для необходимых нам для исследования частей речи (имён существительных, имён прилагательных и глаголов) -- слова, принадлежащие к иным частям речи, после обработки Pymorphy2 просто игнорировались, поэтому алгоритм получился довольно громоздким и долго выполняющимся, особенно на таком непомерно объёмном новостном корпусе. Кроме механизма автосохранения прогресса и самого цикла для отслеживания изменений, мной был также реализован формат выходных данных.</a>
>   * <a style="color:#ffffff">Ввиду слишком длительного времени отработки функции на всём объёме данных и быстро исчерпывающихся бесплатных ресурсов Colab статистика была составлена примерно на 1/3 от общего количества текстов в датасете (222 565 фрагментов из 699 746). Совместно с нашим руководителем П.В. Гращенковым мы коллегиально пришли к выводу, что этого количества информации вполне достаточно для выявления общих исследовательских закономерностей.</a>
>   * <a style="color:#ffffff">наши коллеги -- Анна Жуковская и Алсу Закирова -- проделывали операции схожего рода над гораздо менее объёмным датасетом. Настолько менее объёмным, что по окончании его полной обработки у них получились списки всего из 1704 существительных, 722 глаголов и 490 прилагательных.</a>
>   * <a style="color:#ffffff">Именно эти числа были взяты за</a>
